...
NVIM_WALLPAPER_MODULE = ROOT / "nvim" / ".config" / "nvim" / "lua" / "phajas" / "colors" / "palette_wallpaper.lua"
...
WALLPAPER_FAMILY = "from_wallpaper"
...
def split_wallpaper_palette(palette: dict, family_name: str = WALLPAPER_FAMILY) -> tuple[dict, dict]:
    """Split wallpaper variants into a separate palette structure for editor outputs."""
    families = palette.get("families") or {}
    ...
    for name, payload in families.items():
        if name == family_name:
            wallpaper_families[name] = payload
        else:
            base_families[name] = payload

    base_variants = {}
    wallpaper_variants = {}
    prefix = f"{family_name}_"
    for name, payload in variants.items():
        variant_family = payload.get("family") if isinstance(payload, dict) else None
        if variant_family == family_name or name.startswith(prefix):
            wallpaper_variants[name] = payload
        else:
            base_variants[name] = payload

    base = dict(palette)
    base["families"] = base_families
    base["variants"] = base_variants

    wallpaper = {
        "families": wallpaper_families,
        "variants": wallpaper_variants,
    }
    return base, wallpaper


def lua_wallpaper_merge_lines() -> list[str]:
    return [
...
def get_license_header(variant_name: str, variant: dict, families: dict) -> str:
    ...
    if family_name == "catppuccin":
        return "\n".join([
            ...
            "# License: MIT (see colors/LICENSE-catppuccin)",
    ...
    elif family_name == "phajas":
        license_note = "# License: Custom"
    else:
...
def write_nvim_wallpaper_palette(data: dict) -> bool:
    lua_table = lua_serialize(data)
    content = "\n".join(
        [
            "-- Generated by colors/build.py from colors/palette.toml",
            "-- Wallpaper palette data for Lua consumers (generated)",
            "return " + lua_table,
            "",
        ]
    )
    return write_if_changed(NVIM_WALLPAPER_MODULE, content)
...
def main() -> int:
    ...
    nvim_palette, wallpaper_palette = split_wallpaper_palette(palette)
    changed |= write_nvim_palette(nvim_palette)
    changed |= write_nvim_wallpaper_palette(wallpaper_palette)
    changed |= write_nvim_colorscheme(nvim_palette)
