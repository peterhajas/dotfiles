...
PALETTE_FILE = ROOT / "colors" / "palette.toml"
...
def load_palette() -> dict:
    ...
    raw_text = PALETTE_FILE.read_text()
    data = tomllib.loads(raw_text) if tomllib else parse_simple_toml(raw_text)
    if "variants" not in data or not isinstance(data["variants"], dict):
        raise PaletteError("Palette file must define a [variants] table.")
    ...
    return {"default_variant": default_variant, "variants": variants, "hosts": data.get("hosts", {})}
...
def render_ghostty_variant(file_name: str, variant_name: str, variant: dict) -> str:
    lines = [
...
def ghostty_theme_name(variant_name: str, variant: dict) -> str:
...
def write_ghostty_palettes(variants: dict) -> bool:
    changed = False
    seen = {}
    for name, variant in variants.items():
        file_name = ghostty_theme_name(name, variant)
        ...
        rendered = render_ghostty_variant(file_name, name, variant)
...
def main() -> int:
    ...
    changed = False
    changed |= write_ghostty_palettes(palette["variants"])
